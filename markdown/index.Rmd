...
output: 
  bookdown::pdf_document2:
    ...
    includes:
      in_header: preamble.tex # latex packages and custom commands
---

```{r index_setup, include=FALSE}
packages <- c("openxlsx", "tinytex", "bookdown", "knitr", "gtsummary", "xtable", "stargazer", "kableExtra", "data.table", "gridExtra", "grid", "ggrepel", "ggcorrplot", "tidyverse", "lamadex")
...
Existing indices of youth well-being, such as Youth Progress Index and the Youth Development Index, have been shown to be closely correlated with GDP per capita, especially at lower income levels [@commonwealthsecretariat2016; lisney2018]. Thus, one might expect youth labor market conditions also to rise with incomes and productivity. We test this assumption directly by regressing YLILI on the logarithm of GDP per capita. Column (3) of Table \@ref(tab:tbl-macrocorr) shows that a 1 percent increase in GDP per capita is indeed associated with a 6.4 percent increase in labor market performance for youth. The relationship between the two measures is shown in Figure \@ref(fig:indexgdp) in the Appendix. However, we note that the correlation with GDP is much weaker for the YLILI ( R$^(2)=$ 0.377) than for the more holistic Youth Progress Index (R$^(2)$=0.857). 

## The YLILI and Demographic Change

\noindent The growth in the absolute number of youth has increased much faster in LICs and LMICs than in richer countries (Figure \@ref(fig:youthpop)). At 60\% of the total population, the share of youth below 25 in LICs is double that of HICs and rising (Figure \@ref(fig:stacked) in section \@ref(demographics) of the Appendix). This youth population boom is driven primarily by demographic change in Africa, which has been underway for decades thanks in large part to plummeting child mortality [@ortiz-ospina2016]. The under-20 population in Africa increased by 25.6 percent between 2009 and 2019, and is expected to outnumber the remaining population on the continent by 2070 [@africandevelopmentbank2019]. African populations are already significantly younger than in the rest of the world (Appendix Figure \@ref(fig:pyramids), with a median age, currently at around 18 years, that is unlikely to exceed 21 before 2035. By comparison, by this time, the median person in the world will be aged 35, and the median person in East Asia will be as old as 45 [@filmer2014].

```{r youthpop, fig.align = "center", out.width = '49%', fig.show='hold', fig.cap="World youth population, 1960-2020 \\\\ \\textit{\\footnotesize{Source: World Population Prospects, United Nations (2019)}}"}
...
\noindent All of the component indicators of the YLILI are disaggregated by gender, allowing us to make cross-country comparisons of male- and female-specific YLILI scores, as well as analyze YLILI gender gaps within and between countries.
...
Average dimension and indicator scores for the gender-specific YLILI are presented in Table \@ref(tab:gendermeans) in the Appendix. Labor market conditions in LICs and LMICs favor young men by an average of just under 3 points, as shown in Figure \@ref(fig:gendercorr). There is a negative and marginally significant correlation between the gender difference in YLILI score and the aggregated YLILI itself: countries in which women are most disadvantaged are Afghanistan (ranked 51st out of 54 overall on the YLILI), India (14th), Senegal (36th), and Pakistan (34th), while the Philippines (10th overall), Lesotho (20th), Nicaragua (16th), Honduras (25th) and Ghana (28thst) are among the countries with a higher female than male YLILI (see Tables \@ref(tab:gendiff) and \@ref(tab:gendiffdims) in the online Appendix for country-by-country performance and Figures \@ref(fig:gendiff) and \@ref(fig:genderdims) for a visual representation). We find no relationship between the national gender gap and GDP per capita.

The dimension scores for male and females are shown in the bottom panel of Figure \@ref(fig:genderdims). The gender differences are largest on the transition dimension (left panel), with a mean transition score of 82.3 for men and 75.56 for women. The gender gap in transition scores is driven primarily by high inactivity among young women: the average NEET rate for female youth in our sample is 33.5 percent, compared to 17.7 percent for males--- significantly higher than the global gap (31.1 percent for females versus 13.9 percent for males [@ilo2020]). Developing countries with the most exacerbated gender differences in the NEET rate are Afghanistan, Yemen, Pakistan, Bangladesh, and India, suggesting that regional cultural attitudes play an important role in determining female labor market participation.

```{r fig-gendercorr, fig.align = "center", fig.height=6, fig.pos='H', fig.cap="YLILI score by gender"}
```


\noindent Conditional upon entering the labor market, young women appear to enjoy a higher quality of work than young men, scoring 0.74 points higher on the working conditions dimension. Countries at the top of the YLILI rankings in particular have lower rates of female informal and elementary work (Table \@ref(tab:gendiffdims) in the online Appendix). However, these rates only pertain to youth participating in the labor market, and thus neglect the substantial numbers of women working as caregivers or unpaid household laborers [@ilo2020], who are reflected instead in higher NEET rates for women.
...
\vspace*{5pt}
\noindent where $s_{idc}$ represents country $c$'s score for indicator $i$---which ranges from 0 to 100---for indicator $i$ from dimension $d$. $\text{value}_{idc}$ denotes country $c$'s observed value for indicator $i$. $\text{min}_{id}$ is the value of indicator $i$ at, or below which the score is 0. $\text{max}_{id}$ is the value at, or above which the score is 100.
...
```{r fig-indexgdp, fig.align = "center", fig.height=4, fig.pos='H', fig.cap="Correlation, YLILI score and GDP per capita"}
```

```{r fig-indexunemp, fig.align = "center", fig.height=4, fig.pos='H', fig.cap="Correlation, YLILI score and national youth unemployment rate"}
```

```{r fig-cormat, fig.align = "center", fig.height=8, fig.pos='H', fig.cap="Correlation matrix (× = insignificant at p < 0.05)"}
```

\clearpage

## Demographics by Income Level {#demographics}

```{r stacked, fig.align = "center", out.width = '49%', fig.show='hold', fig.cap="Share of world population by age group, 1960-2020 \\\\ \\textit{\\footnotesize{Source: World Population Prospects, United Nations (2019)}}"}
...
```{r pyramids, fig.align = "center", out.width = '49%', fig.show='hold', fig.cap="Population pyramids for sub-Saharan Africa (left) vs. the world (right) \\\\ \\textit{\\footnotesize{Source: World Population Prospects, United Nations (2019)}}"}
...
```{r worldmap, echo=FALSE, fig.align = "center", out.width = '100%', fig.cap = "Total YLILI depicted by country"}
...
```{r gendiff, echo=FALSE, fig.align = "center", out.width = '100%', fig.cap = "Gender differences in YLILI score"}
knitr::include_graphics("figures/maps/diff_YLILI.png")
```

```{r genderdiffmaps, echo=FALSE, fig.align = "center", out.width = '69%', fig.show='hold', fig.cap="Dimensions of the YLILI depicted by country"}
...
\setcounter{page}{\numexpr\value{savepage}+2}

```{r fig-arithgeom, fig.align = "center", fig.height=4, fig.pos='H', fig.cap="Comparison of aggregation methods"}
```

```{r fig-imputedraw, fig.align = "center", fig.height=4, fig.pos='H', fig.cap="YLILI with and without imputation"}
```
